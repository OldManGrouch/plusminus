		...
		for (int i = 1; i < entityList->vals->size; i++) {
			...
			if (strstr(buff, xorstr("player.prefab"))) {
				...
				if (vars::players::chams && lol) {
					if (lol->health() > 0) {
						uintptr_t playermodel = read(Entity + oPlayerModel, uintptr_t);
				...
				if (vars::misc::auto_revive || vars::misc::insta_revive) {
					...
					if (vars::misc::auto_revive && (BasePlayer*)Entity && lol->HasFlags(64) && Math::Distance_3D(local, utils::GetEntityPosition(gameObject)) < 3.f) {
						lol::PickupPlayer((BasePlayer*)Entity);
					}
					if (vars::misc::insta_revive && (BasePlayer*)Entity && lol->HasFlags(64) && Math::Distance_3D(local, utils::GetEntityPosition(gameObject)) < 3.f && LocalPlayer::Entity()->GetKeyState(Button::USE)
